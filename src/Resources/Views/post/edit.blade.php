...
@section('content')
    <div class="container mt-5">
        ...
        @foreach($posts as $post)
            ...
            @if($post['type'] == 'text')
                <h3 class="text-end mb-4">متن</h3>
                <div class="d-flex">
                    <textarea data-type="text" class="form-control my-2 myElm">{{ json_decode($post['value']) }}</textarea>
                    <span onclick="removeElm(this.parentElement)" class="btn-close"></span>
                </div>
            ...
            @elseif($post['type'] == 'video')
                <h3 class="text-end mb-4">ویدیو</h3>
                <div class="d-flex">
                    <input type="text" data-type="video" class="form-control my-2 myElm" value="{{ json_decode($post['value']) }}">
                    <span onclick="removeElm(this.parentElement)" class="btn-close"></span>
                </div>
            ...
            @elseif($post['type'] == 'title')
                <h3 class="text-end mb-4">عنوان</h3>
                <div class="d-flex">
                    <input type="text" data-type="title" class="form-control my-2 myElm" value="{{ json_decode($post['value']) }}">
                    <span onclick="removeElm(this.parentElement)" class="btn-close"></span>
                </div>

            @elseif($post['type'] == 'list')
                <h3 class="text-end mb-4">لیست</h3>
                <div class="d-flex"><textarea data-type="list" class="form-control my-2 myElm">@foreach(json_decode($post['value']) as $item){{ $item }}@if(!$loop->last)&#10;@endif‌@endforeach</textarea><span onclick="removeElm(this.parentElement)" class="btn-close"></span></div>
    ...
    @endforeach
        ...
        <div class="d-flex justify-content-between">
            <div>
                <button type="button" class="btn btn-warning m-4" data-bs-toggle="modal" data-bs-target="#toolsModal">+</button>
            </div>
            <div>
                <button onclick="update()" type="button" class="btn btn-warning m-4">
                    Edit
...
@section('js')
    <script>
        ...
        function update(){
            ...
            $.ajax({
                url: 'http://{{ $_SERVER['HTTP_HOST'] }}/update',
